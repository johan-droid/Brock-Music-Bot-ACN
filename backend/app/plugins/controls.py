...
from app.utils.permissions import require_admin, require_member, rate_limit
...
from app.plugins.play import persist_playback_state
...
async def forceresume_cmd(client: Client, message: Message):
    ...
    try:
        ...
        from app.plugins.play import cleanup_vc_session
...
async def seek_cmd(client: Client, message: Message):
    ...
    try:
        ...
        from app.utils.audio_config import get_audio_optimizer
...
async def userbotjoin_cmd(client: Client, message: Message):
    ...
    if success:
        await message.reply(
            "✅ Voice chat should now be active or already active.\n\n"
            "💡 The userbot (VC guest) does not need to be admin: it will join an active voice chat and stay "
            "in it while music plays. When the voice chat is stopped it will leave on its own.\n"
            "👉 Just run `/play <song>` to start streaming."
        )
    else:
        await message.reply(
            "❌ Could not auto-start the voice chat. The userbot is a guest (not admin), so it cannot create "
            "a voice chat itself. Please check:\n"
            "1) The userbot is a member of the group.\n"
            "2) A group admin has already started a voice chat (or grant the userbot **Manage Video Chats** to auto-start).\n"
            "3) The group allows voice chats.\n"
            "Then run `/play <song>`."
        )
...
async def vcdebug_cmd(client: Client, message: Message):
    ...
    from app.utils.permissions import check_bot_admin, check_userbot_admin
    ...
    text = (
        ...
        "\n💡 Tip: The userbot (VC guest) does not need to be admin. It will stay in an active voice chat "
        "and leave when the voice chat is stopped. Only give it **Manage Video Chats** if you want it to "
        "auto-create the voice chat on /play."
    )
