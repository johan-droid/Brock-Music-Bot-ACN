...
import uvicorn
from app.scripts import admin_app
from pyrogram.client import Client
from pyrogram.types import BotCommand
from app.config_loader import config
from app.utils.metrics import metrics_collector, log_metrics_periodically
...
def _build_bot_commands() -> list[BotCommand]:
    ...
    return [
        ...
        BotCommand("vplay", "Play from VK / Deezer sources"),
        BotCommand("pause", "Pause playback"),
        BotCommand("resume", "Resume playback"),
        BotCommand("forceresume", "Force-resume (admin)"),
        BotCommand("stop", "Stop playback and leave VC"),
        BotCommand("replay", "Restart the current track"),
        BotCommand("seek", "Seek within the current track"),
        BotCommand("volume", "Set playback volume (0-200)"),
        # ── Queue ──
        ...
        BotCommand("remove", "Remove a queued track by position"),
        BotCommand("move", "Move a track to a new position"),
        BotCommand("clearqueue", "Clear the entire queue"),
        # ── Discovery ──
        BotCommand("vibe", "Search music by mood or vibe"),
        BotCommand("moodsearch", "Search by mood tags"),
        BotCommand("mooddiscovery", "Browse mood suggestions"),
        # ── Playlists ──
        BotCommand("plcreate", "Create a setlist"),
        BotCommand("pladd", "Add a track to a setlist"),
        BotCommand("plremove", "Remove a track from a setlist"),
        BotCommand("pllist", "List your saved setlists"),
        BotCommand("plplay", "Perform a saved setlist"),
        BotCommand("plcollab", "Toggle collaborative mode"),
        BotCommand("plshare", "Share a setlist with the group"),
        BotCommand("plsync", "Sync setlist changes"),
        # ── Radio Shows ──
        BotCommand("showcreate", "Create a radio show slot"),
        BotCommand("showadd", "Add a track to a show"),
        BotCommand("showlist", "List scheduled shows"),
        BotCommand("showpreview", "Preview a show tracklist"),
        BotCommand("showcancel", "Cancel a scheduled show"),
        BotCommand("showhistory", "View past show broadcasts"),
        BotCommand("showtalk", "Open the show talk segment"),
        # ── Song Hunter ──
        BotCommand("starthunter", "Start a song-guessing game"),
        BotCommand("stophunter", "End the current game"),
        BotCommand("hunterboard", "View the game leaderboard"),
        # ── Voting / Anonymous ──
        BotCommand("votemode", "Toggle voting mode (admin)"),
        BotCommand("anonplay", "Request a song anonymously"),
        # ── Effects & Timers ──
        BotCommand("effects", "Open the audio effects menu"),
        BotCommand("sleep", "Set a sleep timer"),
        BotCommand("cancelsleep", "Cancel the sleep timer"),
        ...
        BotCommand("serverhealth", "Check the music server"),
        BotCommand("userbotjoin", "Join or create voice chat"),
        BotCommand("vcdebug", "Inspect voice chat state"),
        # ── Admin ──
        BotCommand("addsudo", "Grant sudo access (owner)"),
        BotCommand("delsudo", "Revoke sudo access (owner)"),
        BotCommand("sudolist", "List sudo users"),
        BotCommand("gban", "Globally ban a user"),
        BotCommand("ungban", "Remove a global ban"),
        BotCommand("block", "Block a user in this group"),
        BotCommand("unblock", "Unblock a user"),
        BotCommand("stats", "Show bot statistics"),
        BotCommand("broadcast", "Broadcast a message (owner)"),
