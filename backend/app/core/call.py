...
from app.utils.ffmpeg import validate_stream_ffprobe
...
class CallManager:
    ...
    async def _on_kicked(self, chat_id: int) -> None:
        """Userbot was kicked from VC — clean up all state."""
        await self._cleanup_call_session(chat_id)
        try:
    ...
    async def _on_closed(self, chat_id: int) -> None:
        """VC was closed by a group admin — leave the call like normal music bots."""
        await self._cleanup_call_session(chat_id)
        try:
            from app.core.queue import queue_manager
    ...
    async def _cleanup_call_session(self, chat_id: int) -> None:
        """Disconnect the userbot from the (now closed/kicked) VC and clear bookkeeping."""
        userbot_idx = self.active_chats.pop(chat_id, None)
        if userbot_idx is None:
            return
        call = self.calls.get(userbot_idx)
        if call:
            try:
                await call.leave_call(chat_id)
                logger.info(f"Userbot left VC in chat {chat_id} after call ended")
            except Exception as exc:
                logger.debug(f"leave_call during cleanup failed for chat {chat_id}: {exc}")
