import asyncio
from app.core.microservice_client import MusicMicroserviceClient, _normalize_urls
...
@pytest.mark.asyncio
async def test_request_keeps_using_primary_endpoint_without_failover():
    client = MusicMicroserviceClient(
    ...
    assert first == []
    assert second == []
    assert all(url.startswith("https://primary-ms") for url in fake_session.calls)
...
@pytest.mark.asyncio
async def test_single_timeout_returns_empty_without_cold_start_retry():
    client = MusicMicroserviceClient(
        base_urls=["https://music-ms.onrender.com"],
        timeout_seconds=5,
    ...
    assert items == []
    assert timeouts_used == [5, 5]


def test_cold_start_detection_disabled():
