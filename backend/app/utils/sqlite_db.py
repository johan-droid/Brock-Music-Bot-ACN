...
class SQLiteDatabase:
    ...
    async def get_quiz_score(self, user_id: int) -> dict:
        """Get user quiz score."""
        conn = self._get_conn()
        row = conn.execute("SELECT score FROM quiz_scores WHERE user_id = ?", (user_id,)).fetchone()
        if row:
            return {"_id": user_id, "score": row["score"]}
    ...
    async def save_quiz_score(self, user_id: int, score_added: int) -> dict:
        ...
        conn = self._get_conn()
        conn.execute(
            "INSERT INTO quiz_scores (user_id, score, updated_at) VALUES (?, ?, CURRENT_TIMESTAMP) "
            "ON CONFLICT(user_id) DO UPDATE SET score = quiz_scores.score + ?, updated_at = CURRENT_TIMESTAMP",
    ...
    async def get_top_quiz_scores(self, limit: int = 10, user_ids: list = None) -> list:
        ...
        if user_ids:
            ...
            rows = conn.execute(
                f"SELECT user_id, score FROM quiz_scores WHERE user_id IN ({placeholders}) ORDER BY score DESC LIMIT ?",
                user_ids + [limit],
            ).fetchall()
        else:
            rows = conn.execute(
                "SELECT user_id, score FROM quiz_scores ORDER BY score DESC LIMIT ?",
                (limit,),
            ).fetchall()
        return [{"_id": row["user_id"], "score": row["score"]} for row in rows]
