...
class NeonDatabase:
    ...
    async def get_quiz_score(self, user_id: int) -> dict:
        ...
        try:
            with self.conn.cursor() as cur:
                cur.execute("SELECT score FROM quiz_scores WHERE user_id = %s", (user_id,))
                row = cur.fetchone()
                if row:
                    return {"_id": user_id, "score": row[0]}
                return {"_id": user_id, "score": 0}
        except Exception as e:
            logger.debug(f"get_quiz_score error: {e}")
            return {"_id": user_id, "score": 0}
    ...
    async def save_quiz_score(self, user_id: int, score_added: int) -> dict:
        ...
        try:
            with self.conn.cursor() as cur:
                cur.execute(
                    "INSERT INTO quiz_scores (user_id, score, updated_at) VALUES (%s, %s, CURRENT_TIMESTAMP) "
                    "ON CONFLICT (user_id) DO UPDATE SET score = quiz_scores.score + %s, updated_at = CURRENT_TIMESTAMP",
                    (user_id, score_added, score_added),
                )
                self.conn.commit()
                return await self.get_quiz_score(user_id)
        except Exception as e:
            self.conn.rollback()
            logger.error(f"save_quiz_score error: {e}")
            return await self.get_quiz_score(user_id)
        ...
        try:
            with self.conn.cursor() as cur:
                if user_ids:
                    ...
                    rows = cur.execute(
                        f"SELECT user_id, score FROM quiz_scores WHERE user_id IN ({placeholders}) ORDER BY score DESC LIMIT %s",
                        user_ids + [limit],
                    ).fetchall()
                else:
                    rows = cur.execute(
                        "SELECT user_id, score FROM quiz_scores ORDER BY score DESC LIMIT %s",
                        (limit,),
                    ).fetchall()
                return [{"_id": row[0], "score": row[1]} for row in rows]
        except Exception as e:
            logger.debug(f"get_top_quiz_scores error: {e}")
            return []
